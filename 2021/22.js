...
const testCaseInput3 = `on x=-5..47,y=-31..22,z=-19..33
...
// const testCase3Map = initMap(50);
// const testCase3Complete = runSteps(testCase3Map, testCaseInput3);
// console.assert(countLitCells(testCase3Complete) === 474140, 'test case 3 failed');
...
// Keep a 3d array of the lit cells
// When necessary, split the arrays
// Might need as many as 6 to do the union or exclusion of cells?
// Maybe do all the turn-ons first and then the turn offs?

// Convert a string into a range
const createRange = (numberString) => {
  const numberStrings = numberString.split(',');
  const numbers = numberStrings.reduce((nums, oneNum) => {
    const letter = oneNum.substring(0, 1);
    const onlyNums = oneNum.split('=')[1];
...
const remove = (onRegion, offRegion) => {

}

// Do two regions overlap?
const doRegionsOverlap = (r1, r2) => {
  
}

// Return true if A is between B and C
const isNumberBetween = (a, b, c) => a >= b && a <= c;

const range1 = createRange('x=10..12,y=10..12,z=10..12');
const range2 = createRange('x=11..13,y=11..13,z=11..13');
